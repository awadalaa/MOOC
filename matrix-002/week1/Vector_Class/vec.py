...
def equal(u,v):
    ...
    """
    assert u.D == v.D
    if sum(u.f.values()) + sum(v.f.values()) == 0:
        return True
    _u = u.f.copy()
    _v = v.f.copy()
    _u.update({ i:0 for i in set(u.D) ^ set(_u.keys())})
    _v.update({ i:0 for i in set(v.D) ^ set(_v.keys())})
    return _u == _v
...
def add(u,v):
    ...
    _dict = {}
    for i in _set:
        _v_value = v[i] if i in v.f else 0
        _u_value = u[i] if i in u.f else 0
        _dict[i] = _u_value + _v_value
    return Vec(_set,_dict)
...
def dot(u,v):
    ...
    12
    """
    return sum([ u[k]*v[k] for k in u.f.keys() if k in v.f])
...
def scalar_mul(v, alpha):
    ...
    True
    """
    return Vec(v.D.copy(),dict(zip(v.f.keys(),[alpha*v[k] for k in v.f])))
