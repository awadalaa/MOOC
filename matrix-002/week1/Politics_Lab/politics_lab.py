...
def create_voting_dict(strlist):
    ...
    for senator in strlist:
        voting_record=[]
        senator_records=senator.split()
        lastname=senator_records[0]
        voting_record= [int(x) for x in senator_records[3:]]
        _dict[lastname]=voting_record
    return  _dict
...
def most_similar(sen, voting_dict):
    ...
    for senator in voting_dict.keys():
        if senator!=sen and max_match<policy_compare(sen,senator,voting_dict):
            max_sen=senator
            max_match=policy_compare(sen,senator,voting_dict)
    return max_sen
...
def least_similar(sen, voting_dict):
    ...
    """
    min_sen='nobody'
    least_match=float("inf")
    for senator in voting_dict.keys():
        if senator!=sen and least_match>policy_compare(sen,senator,voting_dict):
            min_sen=senator
            least_match=policy_compare(sen,senator,voting_dict)
    return min_sen
